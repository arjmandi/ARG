...
def cleanup(
    ...
    if card_id:
        ...
        # Always run agent cleanup, even if the scorecard close failed —
        # recorders and sessions must be finalized regardless.
...
class RedactSecrets(logging.Filter):
    """Strip API keys from log lines (litellm logs request URLs verbatim,
    which would otherwise persist the Gemini key into logs.log)."""

    _pat = re.compile(r"(key=)[A-Za-z0-9_\-]{8,}")
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        try:
            ...
            return True
        redacted = self._pat.sub(r"\1REDACTED", msg)
        if redacted != msg:
            record.msg = redacted
            record.args = ()
        return True
