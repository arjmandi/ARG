...
These have different signatures, different repairs, and — we will show — can be turned on and off independently. Two controlled results from a predecessor system fixed the target [5]. First, a binding existence-proof: a prose fact ("plate at (20,31)") and the matching rendered content (a 3-cell component at (21,31)) never joined across ~2,300 actions and 13 runs of a frozen mid-tier model, until a single mechanical coordinate join was added — which flipped first-level completions from 0 to 3/3 on an otherwise identical stack. Second, a commitment existence-proof: a mid-tier run wrote the winning move verbatim at turns 150 and 184, wrote its own unstick prescription at turn 60, and executed none of them across 171 consecutive turns. The intention was present, recent, and salient; what was missing was an external store executed by code. Prompt rhetoric restored neither capability.
...
3. **An honest efficacy null (§5.4).** No cell completes a level. We pre-registered this as a defeater and report it first, not last.
