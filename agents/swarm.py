...
class Swarm:
    ...
    def close_scorecard(self, card_id: str) -> Optional[Scorecard]:
        ...
        # Only discard the card_id once the close actually succeeded, so a
    ...
    def cleanup(self, scorecard: Optional[Scorecard] = None) -> None:
        """Cleanup all agents. One agent's failure must not skip the others."""
        for a in self.agents:
            try:
                a.cleanup(scorecard)
            except Exception as e:
                logger.warning(f"Cleanup failed for {a.name}: {e}")
