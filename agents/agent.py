...
class Agent(ABC):
    ...
    def do_action_request(self, action: GameAction) -> Response:
        ...
        try:
            ...
            logger.warning(
                f"Non-JSON response during action request: {r.status_code} - {r.text[:200]}"
            )
        return r
    ...
    def take_action(self, action: GameAction) -> Optional[FrameData]:
        ...
        try:
            ...
            logger.warning(f"Action request failed, skipping frame: {e}")
            return None
        try:
            ...
            if hasattr(self, "recorder") and not self.is_playback:
                if scorecard:
                    ...
                    try:
                        ...
                        logger.warning(f"Could not fetch final scorecard: {e}")
                logger.info(
