...
class GameState(str, Enum):
    ...
    # Per API spec: session has ended (WIN or GAME_OVER) and requires RESET.
...
class Scorecard(BaseModel):
    ...
    # Current API spec returns per-game stats under `environments` (one entry
    # per game, each with `runs` per RESET/play). Kept loosely typed so no
    # server field is silently dropped — previously this model discarded it,
    # making every printed scorecard report show zeros.
    environments: list[dict[str, Any]] = Field(default_factory=list)
    total_levels_completed: Optional[int] = None
    ...
    def won(self) -> int:
        ...
        return sum(1 for e in self.environments if e.get("completed"))
    ...
    def total_actions(self) -> int:
        ...
        return sum(e.get("actions", 0) for e in self.environments)

    @computed_field(return_type=int)
    def score(self) -> int:
        if self.cards:
            return sum(g.high_score for g in self.cards.values())
        return sum(e.get("score", 0) for e in self.environments)
...
class FrameData(BaseModel):
    ...
    # Diagnostic fields per current API spec (captured for logging/recordings
    # and cross-matching only — NOT fed into the agent's prompts).
    levels_completed: Optional[int] = None
    win_levels: Optional[int] = None
