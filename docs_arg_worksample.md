...
The design target came from our predecessor system, Sensi ([arXiv 2603.17683](https://arxiv.org/abs/2603.17683)), which diagnosed a *self-consistent hallucination cascade*: errors in its perception layer (frame differencing) propagate through the hypothesis pipeline into internally coherent but factually wrong world models. In the present taxonomy that perception-layer cascade is **binding failure** — a referent grounded in the model's own text rather than in observation. Its counterpart, **commitment failure** — an agent losing its own objective across a long interaction even when the objective is short, recent, and salient — is the goal-drift phenomenon the agent literature measures ([arXiv 2505.02709](https://arxiv.org/abs/2505.02709), [arXiv 2603.03258](https://arxiv.org/abs/2603.03258)). Prompt rhetoric ("pay attention to X") repairs neither. The design conclusion: externalize both mechanisms, separately, and measure them separately.
...
The headline first: across the live campaign to date — 52 runs, ~17M tokens, two protocol days — there are **zero level completions and zero score in every run, including every baseline cell**. The efficacy prediction (P0) is open. What the campaign has validated are mechanism results, each earned under the gates above.
...
| 7 | Cold-start reporting | Pinned and observed | an empty store reports total DEFICIT with empty-evidence holes on every generative run |
